...
static void smb_status_change_work(struct work_struct *work)
{
	...
	if (!regmap_read(chip->regmap,
			 chip->base + BATTERY_CHARGER_STATUS_1, &chg_stat) &&
	    (chg_stat & BATTERY_CHARGER_STATUS_MASK) == INHIBIT_CHARGE) {
		dev_warn(chip->dev,
			 "FSM in INHIBIT_CHARGE, performing ICL-override restart\n");
		/* Step 1: force ICL to 500 mA, bypass AICL result */
		regmap_write(chip->regmap, chip->base + USBIN_CURRENT_LIMIT_CFG,
			     SDP_CURRENT_UA / chip->current_step_size_ua);
		regmap_update_bits(chip->regmap, chip->base + CMD_ICL_OVERRIDE,
				   ICL_OVERRIDE_BIT, ICL_OVERRIDE_BIT);
		msleep(50);
		/* Step 2: disable then re-enable charging to cycle the FSM */
		regmap_update_bits(chip->regmap, chip->base + CHARGING_ENABLE_CMD,
				   CHARGING_ENABLE_CMD_BIT, 0);
		msleep(100);
		regmap_update_bits(chip->regmap, chip->base + CHARGING_ENABLE_CMD,
				   CHARGING_ENABLE_CMD_BIT, CHARGING_ENABLE_CMD_BIT);
		/* Step 3: wait for FSM to stabilize, then hand back to AICL */
		msleep(500);
		regmap_update_bits(chip->regmap, chip->base + CMD_ICL_OVERRIDE,
				   ICL_OVERRIDE_BIT, 0);
	}
...
static const struct smb_init_register smb5_init_seq[] = {
	...
	/* Enable APSD (Automatic Power Source Detection) */
	{ .addr = USBIN_LOAD_CFG,
	  .mask = APSD_EN_BIT,
	  .val = APSD_EN_BIT },
...
static int smb_probe(struct platform_device *pdev)
{
	...
	if (chip->batt_info->voltage_max_design_uv == -EINVAL) {
		dev_warn(chip->dev,
			 "voltage-max-design-microvolt missing, defaulting to 4350000 uV\n");
		chip->batt_info->voltage_max_design_uv = 4350000;
	}
